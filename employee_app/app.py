...
def employee_profiles():
    ...
    if request.method == 'POST':
        ...
        try:
            cursor.execute("""
                UPDATE employees
                SET first_name = %s, last_name = %s, email = %s, contact = %s, role = %s,
                WHERE id = %s
            """, (first_name, last_name, email, contact, role, employee_id))
            conn.commit()
...
def orders():
    ...
    if search_query:
        ...
        query = """
            ...
            FROM orders
            WHERE (CONCAT(first_name, ' ', last_name) LIKE %s OR first_name LIKE %s OR last_name LIKE %s)
...
def order_history():
    ...
    query = """
        SELECT id, first_name, last_name, street_address, city, zip_code, 
               ...
               DATE_FORMAT(created_at, '%M %d, %Y %h:%i %p') AS placed_date, 
               DATE_FORMAT(delivered_at, '%M %d, %Y %h:%i %p') AS delivered_date,
               DATE_FORMAT(updated_at, '%M %d, %Y %h:%i %p') AS last_updated
        FROM history_orders
